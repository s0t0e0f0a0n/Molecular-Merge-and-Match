...
import { LoadingExerciseOverlay } from './LoadingExerciseOverlay';
...
function molBlockWithoutMapNumbers(graph: MolGraph): string {
  ...
  const handleValidateCasAnswer = useCallback(
    ...
    [casAnswerInput, selectedExerciseId],
  );
  return (
    <div
      style={{
          ...
          >
            ...
            >
              <span>
                {loadingExerciseSummaries
                  ? 'Loading…'
                  : (exerciseSummaries.find((e) => e.id === selectedExerciseId)?.name ?? 'Exercises')}
              </span>
