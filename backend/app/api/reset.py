...
class ResetExercisesRequest(BaseModel):
    ...
    level: Literal["logbook", "workspace", "completion", "progression", "remove"]
...
def reset_exercises(body: ResetExercisesRequest) -> None:
    with get_db() as db:
        for exercise_id in set(body.exercise_ids):
            ...
            if body.level in {"workspace", "completion", "progression", "remove"}:
            ...
            if body.level in {"completion", "progression", "remove"} and exercise is not None:
